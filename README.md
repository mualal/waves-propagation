...
- [Мысленный эксперимент](#мысленный-эксперимент)
...
При прохождении через интерфейс полная энергия в системе почему-то не сохраняется (и для решётки, и для цепочки). При отсутствии интерфейса (в однородной решётке) полная энергия сохраняется.

Почему при наличии интерфейса в компьютерном эксперименте энергия системы уменьшается при переходе волны в решётку с меньшими массами частиц и увеличивается при переходе в решётку с большими массами? Это связано со способом интегрирования (использую leapfrog)? Можно ли реализовать алгоритм, чтобы полная энергия сохранялась?
...
В однородной решётке при частоте, близкой к максимальной (почему трансформация не в ромб?):
...
В однородной решётке при максимальной частоте (уменьшил шаг вывода результатов на график):

<img src="images/01-energy-crit-1.jpg">

И ещё уменьшил шаг вывода результатов на график:

<img src="images/01-energy-crit-0.jpg">
...
- В презентации: что значит резкий и плавный переходы?

- Почему в компьютерных экспериментах при переходе волны через интерфейс полная энергия не сохраняется?

- Почему не удалось получить распространение в виде ромбов (подобно статье)?

- Странное поведение полной энергии при максимально возможной частоте?
