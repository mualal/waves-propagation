...
% initial conditions
n_x = repmat(num_x,length(num_y),1);
n_y = repmat(num_y,length(num_x),1)';
disp = u_0 * exp(-beta_x^2/2 * (n_x*cos(gamma)+n_y*sin(gamma)-...
    n_0*cos(gamma)-v_0*sin(gamma)).^2);
disp = disp .* exp(-beta_y^2/2 * (-n_x*sin(gamma)+n_y*cos(gamma)+...
    n_0*sin(gamma)-v_0*cos(gamma)).^2);
disp = disp .* sin(k_1*a*cos(gamma)*n_x+k_1*a*sin(gamma)*n_y);
vel = -u_0 * exp(-beta_x^2/2 * (n_x*cos(gamma)+n_y*sin(gamma)-...
    n_0*cos(gamma)-v_0*sin(gamma)).^2);
vel = vel .* exp(-beta_y^2/2 * (-n_x*sin(gamma)+n_y*cos(gamma)+...
    n_0*sin(gamma)-v_0*cos(gamma)).^2);
vel = vel .* (omega*cos(k_1*a*cos(gamma)*n_x+k_1*a*sin(gamma)*n_y)-...
...
plot(0:save_time:t_max,cell2mat(cellfun(@(x) sum(sum(x(1:end,...
    1:sum(num_x<0)))),result_e,'UniformOutput',false)),'LineWidth',1,...
    'Color','Blue')
plot(0:save_time:t_max,cell2mat(cellfun(@(x) sum(sum(x(1:end,...
    sum(num_x<=0):length(num_x)))),result_e,'UniformOutput',false)),...
...
e_l = cell2mat(cellfun(@(x) sum(sum(x(1:end,1:sum(num_x<0)))),result_e,...
    'UniformOutput',false));
e_r=cell2mat(cellfun(@(x) sum(sum(x(1:end,sum(num_x<=0):length(num_x)))),...
