...
for i=1:length(num_x)
    for j=1:length(num_y)
        ...
        disp(j, i) = disp(j, i) * sin(k_1*a*cos(gamma)*num_x(i)+k_1*a*sin(gamma)*num_y(j));
        ...
        vel(j, i) = vel(j, i) * (omega*cos(k_1*a*cos(gamma)*num_x(i)+k_1*a*sin(gamma)*num_y(j)) - beta_x^2*g_1/a*(num_x(i)*cos(gamma)+num_y(j)*sin(gamma)-n_0*cos(gamma)+v_drift)*sin(k_1*a*cos(gamma)*num_x(i)+k_1*a*sin(gamma)*num_y(j)));
        %vel(j, i) = vel(j, i) * (omega * cos(k_1*(num_x(i))) - beta_x^2*g_1/a*(num_x(i)-n_0)*sin(num_x(i) * k_1 * a));
    end
end
...
plot3(x_coord,y_coord,max(max(disp_initial))*ones(1,length(x_coord)),'LineWidth',2,'Color','Black')
...
xlabel('Номер частицы по оси Ox');
ylabel('Номер частицы по оси Oy');
l1 = plot3(x_coord,y_coord,10*ones(1,length(x_coord)),'LineWidth',2,'Color','Black');
colorbar;
caxis([-1 1])
%s1.EdgeColor = 'none';
%view(17,22);
